...
public final class LaunchPadPacket {
    ...
    public static void handle(LaunchPadPacket packet, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (!(player.containerMenu instanceof LaunchPadMenu menu) || !menu.stillValid(player)) {
                return;
            }
            BlockEntity be = player.level().getBlockEntity(packet.pos);
            if (be instanceof LaunchPadBlockEntity pad && menu.getBlockEntity() == pad && !pad.isRemoved()) {
                if (pad.launch()) {
