...
/**
 * Expanding teal Solinium sphere cloud (legacy {@code EntityCloudSolinium}).
 */
public class EntityCloudSolinium extends Entity {
    ...
    public EntityCloudSolinium(Level level, int maxAge) {
        this(ModEntities.CLOUD_SOLINIUM.get(), level);
        setMaxAge(maxAge);
    }
    ...
    public void tick() {
        ...
        age++;
        scale++;
        if (!level().isClientSide && level() instanceof ServerLevel server && age % 5 == 0) {
            LightningBolt bolt = EntityType.LIGHTNING_BOLT.create(server);
            if (bolt != null) {
                bolt.moveTo(getX(), getY() + 200.0D, getZ());
