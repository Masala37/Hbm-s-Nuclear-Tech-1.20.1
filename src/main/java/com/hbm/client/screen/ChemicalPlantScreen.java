...
public class ChemicalPlantScreen extends AbstractContainerScreen<ChemicalPlantMenu> {
    ...
    public ChemicalPlantScreen(ChemicalPlantMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (recipe != null) {
            for (int i = 0; i < recipe.inputItem().size() && i < ChemicalPlantBlockEntity.INPUT_SLOTS.length; i++) {
        ...
        for (int i = 0; i < 3; i++) {
            drawFluid(graphics, x + 8 + i * 18, y + 18, 16, 34, this.menu.getInputFluid(i));
            drawFluid(graphics, x + 80 + i * 18, y + 18, 16, 34, this.menu.getOutputFluid(i));
        }
    }
    ...
    private void drawFluid(GuiGraphics graphics, int x, int y, int width, int height, FluidStack fluid) {
        ...
        int h = Math.min(height, fluid.getAmount() * height / ChemicalPlantBlockEntity.TANK_CAPACITY);
        if (h <= 0) {
            return;
        }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (isHovering(80 + i * 18, 18, 16, 34, mouseX, mouseY)) {
                tooltipTank(graphics, this.menu.getOutputFluid(i), mouseX, mouseY);
            }
        }
    ...
    private void tooltipTank(GuiGraphics graphics, FluidStack fluid, int mouseX, int mouseY) {
        if (fluid == null || fluid.isEmpty()) {
            graphics.renderTooltip(this.font,
                    Component.literal("Empty: 0 / " + ChemicalPlantBlockEntity.TANK_CAPACITY + " mB"),
                    mouseX, mouseY);
            return;
        }
        graphics.renderTooltip(this.font,
                Component.literal(fluid.getDisplayName().getString() + ": " + fluid.getAmount() + " / "
                        + ChemicalPlantBlockEntity.TANK_CAPACITY + " mB"),
                mouseX, mouseY);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (isHovering(7, 125, 18, 18, (int) mouseX, (int) mouseY)) {
            ChemicalPlantBlockEntity be = this.menu.getBlockEntity();
            this.minecraft.setScreen(new RecipeSelectorScreen(
                    be.getBlockPos(),
                    0,
                    be.getRecipeName(),
                    AssemblyMachineBlockEntity.grabPool(be.getItems().getStackInSlot(ChemicalPlantBlockEntity.SLOT_BLUEPRINT)),
                    this,
                    RecipeSelectorScreen.CHEMPLANT));
            return true;
        }
