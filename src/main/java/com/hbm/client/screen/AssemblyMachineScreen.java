...
public class AssemblyMachineScreen extends AbstractContainerScreen<AssemblyMachineMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(RefStrings.MODID, "textures/gui/processing/gui_assembler.png");

    public AssemblyMachineScreen(AssemblyMachineMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (recipe != null) {
            for (int i = 0; i < recipe.inputItem().size() && i < AssemblyMachineBlockEntity.INPUT_SLOTS.length; i++) {
        ...
        drawFluid(graphics, x + 8, y + 99, 52, 16, this.menu.getInputFluid());
        drawFluid(graphics, x + 80, y + 99, 52, 16, this.menu.getOutputFluid());
    }
    ...
    private void drawFluid(GuiGraphics graphics, int x, int y, int width, int height, FluidStack fluid) {
        ...
        int w = Math.min(width, fluid.getAmount() * width / AssemblyMachineBlockEntity.TANK_CAPACITY);
        if (w <= 0) {
            return;
        }
        IClientFluidTypeExtensions extensions = IClientFluidTypeExtensions.of(fluid.getFluid());
        int color = extensions.getTintColor(fluid);
        graphics.fill(x, y, x + w, y + height, color == 0 ? 0xFF3F76E4 : color | 0xFF000000);
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (isHovering(8, 99, 52, 16, mouseX, mouseY)) {
            tooltipTank(graphics, this.menu.getInputFluid(), mouseX, mouseY);
        }
        if (isHovering(80, 99, 52, 16, mouseX, mouseY)) {
    ...
    private void tooltipTank(GuiGraphics graphics, FluidStack fluid, int mouseX, int mouseY) {
        if (fluid == null || fluid.isEmpty()) {
            graphics.renderTooltip(this.font,
                    Component.literal("Empty: 0 / " + AssemblyMachineBlockEntity.TANK_CAPACITY + " mB"),
                    mouseX, mouseY);
            return;
        }
        graphics.renderTooltip(this.font,
                Component.literal(fluid.getDisplayName().getString() + ": " + fluid.getAmount() + " / "
                        + AssemblyMachineBlockEntity.TANK_CAPACITY + " mB"),
                mouseX, mouseY);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (isHovering(7, 125, 18, 18, (int) mouseX, (int) mouseY)) {
            AssemblyMachineBlockEntity be = this.menu.getBlockEntity();
            this.minecraft.setScreen(new RecipeSelectorScreen(
                    be.getBlockPos(),
                    0,
                    be.getRecipeName(),
                    AssemblyMachineBlockEntity.grabPool(be.getItems().getStackInSlot(AssemblyMachineBlockEntity.SLOT_BLUEPRINT)),
                    this));
            return true;
        }
