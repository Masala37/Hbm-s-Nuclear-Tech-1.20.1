...
public class OilWellScreen extends AbstractContainerScreen<OilWellMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(RefStrings.MODID, "textures/gui/machine/gui_well.png");

    public OilWellScreen(OilWellMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (energyH > 0) {
            graphics.blit(TEXTURE, x + 8, y + 51 - energyH, 176, 34 - energyH, 16, energyH);
        }
        int k = this.menu.getIndicator();
        if (k != 0) {
            graphics.blit(TEXTURE, x + 35, y + 17, 176 + (k - 1) * 16, 52, 16, 16);
        }
        graphics.blit(TEXTURE, x + 34, y + 36, 192, 0, 18, 34);
        drawTank(graphics, x + 62, y + 17, 16, 52, this.menu.getBlockEntity().getOil().getFluid(),
                this.menu.getOilAmount(), this.menu.getOilCap());
        drawTank(graphics, x + 107, y + 17, 16, 52, this.menu.getBlockEntity().getGas().getFluid(),
                this.menu.getGasAmount(), this.menu.getGasCap());
    }
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (isHovering(8, 17, 16, 34, mouseX, mouseY)) {
            ...
            tooltipTank(graphics, this.menu.getBlockEntity().getOil().getFluid(),
                    this.menu.getOilAmount(), this.menu.getOilCap(), mouseX, mouseY);
        } else if (isHovering(107, 17, 16, 52, mouseX, mouseY)) {
            tooltipTank(graphics, this.menu.getBlockEntity().getGas().getFluid(),
                    this.menu.getGasAmount(), this.menu.getGasCap(), mouseX, mouseY);
        }
    }
