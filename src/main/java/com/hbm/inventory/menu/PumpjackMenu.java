...
public class PumpjackMenu extends AbstractContainerMenu {
    ...
    public PumpjackMenu(int id, Inventory inv, PumpjackBlockEntity be) {
        this(id, inv, be, createData(be));
    }

    public PumpjackMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, NukeMenuHelper.readBlockEntity(inv, buf, PumpjackBlockEntity.class, PumpjackBlockEntity::new),
                new SimpleContainerData(7));
    }

    private PumpjackMenu(int id, Inventory inv, PumpjackBlockEntity be, ContainerData data) {
        super(ModMenus.MACHINE_PUMPJACK.get(), id);
        this.be = be;
        this.data = data;
        this.addSlot(new SlotItemHandler(be.getItems(), PumpjackBlockEntity.SLOT_BATTERY, 8, 53));
        this.addSlot(new SlotItemHandler(be.getItems(), PumpjackBlockEntity.SLOT_OIL_IN, 80, 17));
        this.addSlot(new SlotItemHandler(be.getItems(), PumpjackBlockEntity.SLOT_OIL_OUT, 80, 53) {
            @Override
            public boolean mayPlace(@NotNull ItemStack stack) {
                return false;
            }
        });
        this.addSlot(new SlotItemHandler(be.getItems(), PumpjackBlockEntity.SLOT_GAS_IN, 125, 17));
        this.addSlot(new SlotItemHandler(be.getItems(), PumpjackBlockEntity.SLOT_GAS_OUT, 125, 53) {
            @Override
            public boolean mayPlace(@NotNull ItemStack stack) {
                return false;
            }
    ...
    private static ContainerData createData(PumpjackBlockEntity be) {
