...
public class NukeManMenu extends AbstractContainerMenu {
    private final NukeManBlockEntity be;
    private final ContainerLevelAccess access;

    public NukeManMenu(int id, Inventory inv, NukeManBlockEntity be) {
    ...
    public NukeManMenu(int id, Inventory inv, FriendlyByteBuf buf) {
        this(id, inv, NukeMenuHelper.readBlockEntity(inv, buf, NukeManBlockEntity.class, NukeManBlockEntity::new));
    }
    ...
    public NukeManBlockEntity getBlockEntity() {
        return be;
    }
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            result = stack.copy();
            if (index < NukeManBlockEntity.SLOT_COUNT) {
                if (!this.moveItemStackTo(stack, NukeManBlockEntity.SLOT_COUNT, this.slots.size(), true)) {
                    return ItemStack.EMPTY;
                }
            } else if (!this.moveItemStackTo(stack, 0, NukeManBlockEntity.SLOT_COUNT, false)) {
                return ItemStack.EMPTY;
            }
