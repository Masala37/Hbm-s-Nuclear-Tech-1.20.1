...
/**
 * Shell-expanding Solinium dig (legacy {@code ExplosionSolinium}).
 * Converts organic terrain via {@link ExplosionNukeGeneric#solinium}.
 */
public class ExplosionSolinium {
    ...
    private void breakColumn(int x, int z) {
        ...
        for (int y = (int) (dist / explosionCoefficient2); y > -dist / explosionCoefficient; y--) {
            cursor.set(posX + x, posY + y, posZ + z);
            if (level.isInWorldBounds(cursor)) {
                ExplosionNukeGeneric.solinium(level, cursor.immutable());
            }
        }
    }
