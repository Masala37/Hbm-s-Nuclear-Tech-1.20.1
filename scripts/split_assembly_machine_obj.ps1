...
$src = Join-Path $root "src/main/resources/assets/hbm/models/legacy_raw/machines/assembly_machine.obj"
...
$tex = "hbm:models/machines/assembly_machine"
...
function Emit-Object($obj, [string]$mtllib, [string]$slug) {
...
foreach ($name in $tesr.Keys) {
  ...
  $slug = $tesr[$name]
  Emit-Object $byName[$name] "$slug.mtl" $slug
}
...
[void]$itemLines.Add("mtllib assembly_machine_item.mtl")
...
Assert-Faces $itemText "assembly_machine_item"
Write-Utf8NoBom (Join-Path $outDir "assembly_machine_item.obj") $itemText
Write-Utf8NoBom (Join-Path $outDir "assembly_machine_item.mtl") $mtlBody
Write-Utf8NoBom (Join-Path $blockDir "machine_assembly_machine.json") (Block-Json "assembly_machine_item")
Write-Host "assembly_machine_item.obj objects $($objects.Count) verts $($usedV.Count)"
Write-Host "done"
