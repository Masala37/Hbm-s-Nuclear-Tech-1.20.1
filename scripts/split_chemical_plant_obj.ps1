...
$src = Join-Path $root "src/main/resources/assets/hbm/models/legacy_raw/machines/chemical_plant.obj"
...
$tex = "hbm:models/machines/chemical_plant"
$fluidTex = "hbm:models/machines/chemical_plant_fluid"
$mtlBody = "newmtl material`nKd 1 1 1`nmap_Kd $tex`n"
$fluidMtlBody = "newmtl material`nKd 1 1 1`nmap_Kd $fluidTex`n"
$utf8 = New-Object System.Text.UTF8Encoding $false

$tesr = [ordered]@{
  Base = "chemical_plant_base"
  Frame = "chemical_plant_frame"
  Slider = "chemical_plant_slider"
  Spinner = "chemical_plant_spinner"
  Fluid = "chemical_plant_fluid"
}
...
foreach ($name in $tesr.Keys) {
  ...
  $slug = $tesr[$name]
  if ($name -eq "Fluid") {
    Emit-Object $byName[$name] "$slug.mtl" $slug $fluidTex $fluidMtlBody
  } else {
    Emit-Object $byName[$name] "$slug.mtl" $slug $tex $mtlBody
  }
}

$itemParts = @("Base", "Slider", "Spinner", "Frame")
$vs = New-Object System.Collections.Generic.List[int]
$vts = New-Object System.Collections.Generic.List[int]
$vns = New-Object System.Collections.Generic.List[int]
foreach ($name in $itemParts) {
  $obj = $byName[$name]
  foreach ($face in $obj.faces) {
    foreach ($c in $face) {
      [void]$vs.Add($c.v)
...
[void]$itemLines.Add("mtllib chemical_plant_item.mtl")
[void]$itemLines.Add("usemtl material")
foreach ($index in $usedV) { [void]$itemLines.Add($verts[$index - 1]) }
foreach ($index in $usedVt) { [void]$itemLines.Add($texs[$index - 1]) }
foreach ($index in $usedVn) { [void]$itemLines.Add($norms[$index - 1]) }
foreach ($name in $itemParts) {
  $obj = $byName[$name]
  [void]$itemLines.Add("o $($obj.name)")
  if ($obj.smoothing) { [void]$itemLines.Add($obj.smoothing) }
  foreach ($face in $obj.faces) {
    $bits = @()
    foreach ($c in $face) { $bits += (Format-Corner $c $mapV $mapVt $mapVn) }
    [void]$itemLines.Add("f " + ($bits -join " "))
  }
}
$itemText = ($itemLines -join "`n") + "`n"
Assert-Faces $itemText "chemical_plant_item"
Write-Utf8NoBom (Join-Path $outDir "chemical_plant_item.obj") $itemText
Write-Utf8NoBom (Join-Path $outDir "chemical_plant_item.mtl") $mtlBody
Write-Utf8NoBom (Join-Path $blockDir "machine_chemical_plant.json") (Block-Json "chemical_plant_item" $tex)
Write-Host "chemical_plant_item.obj objects $($itemParts.Count) verts $($usedV.Count)"
Write-Host "done"
